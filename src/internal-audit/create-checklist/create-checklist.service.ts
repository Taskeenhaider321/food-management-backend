...
export class CreateChecklistService {
  constructor(
    ...
    @InjectModel(ChecklistQuestion.name)
  ...
  async addChecklist(createDto: CreateChecklistDto) {
    ...
    for (let idx = 0; idx < createDto.ChecklistQuestions.length; idx++) {
      ...
      const doc = new this.checklistQuestionModel({
        ...
        responseGroup: q.responseGroup
          ? new Types.ObjectId(q.responseGroup)
          : undefined,
        ComplianceType: q.ComplianceType,
        order: q.order ?? idx,
      });
      const saved = await doc.save();
      questionsIds.push(saved._id);
    }
    ...
    return {
      status: true,
      message: 'The checklist is added!',
      data: checklist,
    };
  }
  ...
  async getChecklists(departmentId: string) {
    const checklists = await this.checklistModel
      ...
      .populate({
        ...
        populate: { path: 'responseGroup' },
      });
    return {
      status: true,
      message: 'The following are Checklists!',
      data: checklists,
    };
  }
  ...
  async updateChecklist(updateDto: UpdateChecklistDto) {
    ...
    const isAfterRejection =
      currentChecklist.Status === 'Rejected' ||
      currentChecklist.Status === 'Disapproved';
    ...
    if (updateDto.description !== undefined)
      updatedData.description = updateDto.description;
    if (updateDto.settings)
      updatedData.settings = {
        ...currentChecklist.settings,
        ...updateDto.settings,
      };
  ...
  async deleteChecklist(id: string) {
    ...
    return {
      status: true,
      message: 'The checklist is deleted!',
      data: deleted,
    };
  }
  ...
  async deleteAllChecklists(): Promise<{
    ...
    if (result.deletedCount === 0)
      throw new NotFoundException('No Checklists Found to Delete!');
    return {
      status: true,
      message: 'All Checklists have been Deleted!',
      data: result,
    };
  }
  ...
  async reviewChecklist(dto: ReviewChecklistDto) {
    ...
    if (checklist.Status !== 'In Review') {
      throw new BadRequestException(
        'Checklist can only be reviewed from "In Review" status',
      );
    }
    ...
    return {
      status: true,
      message: 'Checklist has been marked as Reviewed.',
      data: checklist,
    };
  }
  ...
  async approveChecklist(approveDto: ApproveChecklistDto) {
    ...
    if (checklist.Status !== 'Reviewed') {
      throw new BadRequestException(
        'Checklist can only be approved from "Reviewed" status',
      );
    }
    ...
    return {
      status: true,
      message: 'The Checklist has been approved.',
      data: checklist,
    };
  }
  ...
  async rejectChecklist(rejectDto: RejectChecklistDto) {
    ...
    if (!['In Review', 'Reviewed'].includes(checklist.Status)) {
      throw new BadRequestException(
        'Checklist can only be rejected from "In Review" or "Reviewed" status',
      );
    }
    ...
    return {
      status: true,
      message: 'The Checklist has been rejected.',
      data: checklist,
    };
  }
  ...
  async disapproveChecklist(disapproveDto: DisapproveChecklistDto) {
    ...
    if (checklist.Status !== 'Approved') {
      throw new BadRequestException(
        'Checklist can only be disapproved from "Approved" status',
      );
    }
    ...
    return {
      status: true,
      message: 'The Checklist has been disapproved.',
      data: checklist,
    };
  }
  ...
  async updateSettings(checklistId: string, settings: any) {
    ...
    checklist.settings = { ...checklist.settings, ...settings };
    await checklist.save();

    return {
      status: true,
      message: 'Checklist settings updated!',
      data: checklist,
    };
  }
  ...
  async getTimeline(checklistId: string) {
    ...
    return {
      status: true,
      message: 'Status timeline retrieved!',
      data: checklist.statusTimeline,
    };
  }
  ...
  async getResponseGroups() {
    ...
    return {
      status: true,
      message: 'Response groups retrieved!',
      data: groups,
    };
  }
  ...
  async updateResponseGroup(dto: UpdateResponseGroupDto) {
    ...
    if (group.isDefault)
      throw new BadRequestException('Cannot edit default response groups');
    ...
    const updated = await this.responseGroupModel.findByIdAndUpdate(
      dto._id,
      updateData,
      { returnDocument: 'after' },
    );
  ...
  async deleteResponseGroup(id: string) {
    ...
    if (!group) throw new NotFoundException('Response group not found');
    if (group.isDefault)
      throw new BadRequestException('Cannot delete default response groups');
