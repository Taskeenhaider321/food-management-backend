...
export class UploadDocumentsService {
  constructor(
    @InjectModel(UploadDocuments.name)
    private uploadDocumentsModel: Model<UploadDocuments>,
  ...
  private async addFirstPage(
    ...
    page.drawText(this.formatDate(new Date()), {
      x: 330,
      y: yPosition - 80,
      color: rgb(0, 0, 0),
      size: 15,
  ...
  private async addFirstPageWithApproval(
    page: any,
    logoImage: any,
    company: any,
    creatorName: string,
    ...
    page.drawText(this.formatDate(createdDate), {
      x: 330,
      y: yPosition - 80,
      color: rgb(0, 0, 0),
      size: 15,
    ...
    page.drawText(
      reviewDate !== '---' ? this.formatDate(reviewDate) : reviewDate,
      { x: 330, y: yPosition - 170, color: rgb(0, 0, 0), size: 15 },
    );
    ...
    page.drawText(
      approvalDate !== '---' ? this.formatDate(approvalDate) : approvalDate,
      { x: 330, y: yPosition - 230, color: rgb(0, 0, 0), size: 15 },
    );
    page.drawText('Revision No : ', {
      x: 80,
      y: yPosition - 260,
    ...
    pdfDoc
      ...
      .forEach((page) => {
        ...
        page.drawText(newDocument.DocumentName, {
          x:
            width / 2 -
            helveticaFont.widthOfTextAtSize(newDocument.DocumentName, 10) / 2,
        ...
        page.drawText('Revision No : 0', {
          x:
            width - helveticaFont.widthOfTextAtSize('Revision No : 0', 10) - 20,
  ...
  async findAll(departmentId: string) {
    ...
    return {
      status: true,
      message: 'The following are Documents!',
      data: documents,
    };
  }
  ...
  async review(dto: ReviewUploadedDocumentDto) {
    ...
    if (document.Status !== 'Pending')
      throw new BadRequestException(
        'Document status is not eligible for review.',
      );
  ...
  async reject(dto: RejectUploadedDocumentDto) {
    ...
    if (document.Status !== 'Pending' && document.Status !== 'Reviewed') {
      throw new BadRequestException(
        'Document status is not eligible for rejection.',
      );
    }
    ...
    document.UploadedDocuments[
      document.UploadedDocuments.length - 1
    ].ReviewDate = undefined;
    document.UploadedDocuments[
      document.UploadedDocuments.length - 1
  ...
  async approve(dto: ApproveUploadedDocumentDto) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Document status is not eligible for approval.',
      );
    }
  ...
  async disapprove(dto: DisapproveUploadedDocumentDto) {
    ...
    if (
      ...
      throw new BadRequestException(
        'Document status is not eligible for disapproval.',
      );
    }
  ...
  async replace(
    ...
    if (
      ...
      throw new BadRequestException(
        'Document status does not allow replacement',
      );
    }
